...
# CONTINUE: IMPLEMENTING Q-LEARNING ALGO
    ...
    def createQ(self, state):
        ...
        if self.learning:
            stringState = str(state)
            if stringState not in self.Q:
                print 'updating self.Q with ',stringState
                stateDict = dict()
                # self.Q.update(dict(str(state),dict()))
                for action in self.valid_actions:
                    stateDict[action] = 0
                self.Q[stringState] = stateDict
        ###########
        ## TO DO ##
        ###########
    ...
    def learn(self, state, action, reward):
        ...
        stringState = str(state)
        if self.learning:
            oldReward = self.Q[stringState][action]
            self.Q[stringState][action] += ((reward-oldReward)*self.alpha)
            print 'new reward: ',self.Q[stringState][action]
...
def run():
    ...
    sim = Simulator(env, update_delay=.001, log_metrics=True, optimized=False)
